      ...
      <div className='relative overflow-x-auto'>
        <div>
          {user.name && user.surname && (
            <p className='flex flex-none items-center'>
              ...
              <span className="font-bold flex-none ml-3 mr-3">Full name:</span> {user.name} {user.surname}
            </p>
          )}
          {user.email && (
            <p className='flex flex-none py-2'>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 flex-none h-5">
                <path d="M3 4a2 2 0 00-2 2v1.161l8.441 4.221a1.25 1.25 0 001.118 0L19 7.162V6a2 2 0 00-2-2H3z" />
                <path d="M19 8.839l-7.77 3.885a2.75 2.75 0 01-2.46 0L1 8.839V14a2 2 0 002 2h14a2 2 0 002-2V8.839z" />
              </svg>
              <span className="font-bold flex-none ml-3 mr-3">E-mail address:</span> {user.email}
            </p>
          )}
          {user.address && (
            <p className='flex flex-none py-2'>
              ...
              <span className="font-bold flex-none ml-3 mr-3">Address:</span> {user.address}
            </p>
          )}
          {user.birthdate && (
            <p className='flex flex-none py-2'>
              ...
              <span className="font-bold flex-none ml-3 mr-3">Birthdate:</span>{user.birthdate ? new Date(user.birthdate).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''}
            </p>
          )}
        </div>
      </div>
      <div className='flex flex-col'>
      {!orderItems || orderItems.orders === null || orderItems.orders.length === 0 ? (
        <div className='text-center py-10'>
          <span className='text-sm text-gray-400'>You do not have any order.</span>
        </div>
      ) : (
        <>
          <br></br>
